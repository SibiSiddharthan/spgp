...
int32_t kdf_counter_test_suite(void)
{
	...
	ctx.prf = KDF_PRF_HMAC;
	ctx.mode = KDF_MODE_COUNTER;
...
int32_t kdf_double_pipeline_test_suite(void)
{
	...
	ctx.prf = KDF_PRF_HMAC;
	ctx.mode = KDF_MODE_DOUBLE_PIPLELINE;
	ctx.algorithm = HASH_SHA256;
	ctx.counter = KDF_COUNTER_32;
	ctx.flags = KDF_FIXED_DATA;
	ctx.location = KDF_COUNTER_BEFORE;
