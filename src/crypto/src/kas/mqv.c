				...
				bignum_t *static_pulbic_key, bignum_t *ephermal_public_key, void *shared_secret, uint32_t size)
{
	...
	uint32_t w = CEIL_DIV(self_static_key->group->q->bits, 2);
	bignum_t *r = NULL, *s = NULL, *u = NULL, *v = NULL, *pm1 = NULL;

	uint32_t ctx_size = 4 * bignum_size(self_static_key->group->p->bits) + bignum_size(self_static_key->group->q->bits);

	bignum_ctx_start(self_static_key->group->bctx, ctx_size);

	u = bignum_ctx_allocate_bignum(self_static_key->group->bctx, self_static_key->group->p->bits);
	v = bignum_ctx_allocate_bignum(self_static_key->group->bctx, self_static_key->group->p->bits);
	r = bignum_ctx_allocate_bignum(self_static_key->group->bctx, self_static_key->group->p->bits);
	pm1 = bignum_ctx_allocate_bignum(self_static_key->group->bctx, self_static_key->group->p->bits);
	s = bignum_ctx_allocate_bignum(self_static_key->group->bctx, self_static_key->group->q->bits);
	...
	s = bignum_modmul(self_static_key->group->bctx, s, u, self_static_key->x, self_static_key->group->q);
	s = bignum_modadd(self_static_key->group->bctx, s, s, self_ephemeral_private_key, self_static_key->group->q);

	r = bignum_modexp(self_static_key->group->bctx, r, static_pulbic_key, v, self_static_key->group->p);
	r = bignum_modmul(self_static_key->group->bctx, r, r, v, self_static_key->group->p);
	r = bignum_modexp(self_static_key->group->bctx, r, r, s, self_static_key->group->p);

	pm1 = bignum_copy(pm1, self_static_key->group->p);
	...
	bignum_ctx_end(self_static_key->group->bctx);
